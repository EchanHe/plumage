...
class HourglassModel():
    ...
    def generate_model(self):
        ...
        with tf.device(self.cpu):
            with tf.name_scope('accuracy'):
                ...
                self.joint_accur_v2 = tf.placeholder(dtype = tf.float32, shape = (self.outDim,))
        ...
        with tf.device(self.cpu):
            with tf.name_scope('training'):
                ...
                for i in range(self.outDim):
                    tmp = self.gtMaps[0,self.nStack-1, :, :, i] * 255
                    tmp = tf.expand_dims(tmp, axis = 2)
                    tmp = tf.expand_dims(tmp, axis = 0)
                    tf.summary.image('first_label' + str(i), tmp, max_outputs = 100, collections = ['train'])

                    tmp = self.output[0,self.nStack-1, :, :, i] * 255
                    tmp = tf.expand_dims(tmp, axis = 2)
                    tmp = tf.expand_dims(tmp, axis = 0)
                    tf.summary.image('Output' + str(i), tmp, max_outputs = 100, collections = ['train'])

            with tf.name_scope('valid'):
                for i in range(len(self.joints)):
                    tf.summary.scalar(self.joints[i], self.joint_accur_v2[i], collections = ['test'])
                tf.summary.scalar("ave_error", tf.reduce_mean(self.joint_accur_v2), collections = ['test'])
    ...
    def coord_accuracy(self, results,coord):
        ...
        for i in range(df_size):
            for j in range(cnt_size):
                ...
                # if coord[i,j*2] !=-1:
                #     print((map_shape[1],map_shape[0]) , coord[i,j*2:(j+1)*2])
                output_result[i,j*2+0] = map_shape[1] + 1
                output_result[i,j*2+1] = map_shape[0] + 1
                if coord[i,j*2] ==-1:
                    output_result[i,j*2+0] = -1
                    output_result[i,j*2+1] = -1
        output_result2 = np.ones((df_size,cnt_size))
        for j in range(cnt_size):   
            output_result2[:,j] = np.linalg.norm(output_result[:,j*2:j*2+2] - coord[:,j*2:j*2+2] , axis=1)
        # print("result: " )
        # print(output_result2.astype(int))

        for i in range(df_size):
            for j in range(cnt_size):
                if coord[i,j*2] ==-1:
                    output_result2[i,j] = np.NaN    
        acc_per_row = np.nanmean( output_result2 , axis=0)
        #acc_per_row = np.mean( output_result2 , axis=0)
        return acc_per_row  

    def coord_accuracy_v2(self, results,coord , pck_threshold , validation_scale):
        ...
        coord_diff = np.ones((df_size,cnt_size))
        for j in range(cnt_size):   
            coord_diff[:,j] = np.linalg.norm(output_result[:,j*2:j*2+2]//self.img_scale - coord[:,j*2:j*2+2]//self.img_scale , axis=1)

        for i in range(df_size):
            for j in range(cnt_size):
                if coord[i,j*2] ==-1:
                    coord_diff[i,j] = np.NaN    
        acc_per_row = np.nanmean( coord_diff , axis=0)
        pck = np.zeros(cnt_size)
        for j in range(cnt_size):
    ...
    def _train(self, nEpochs = 10, epochSize = 1000, saveStep = 500, validIter = 10):
        ...
        with tf.name_scope('Train'):
            ...
            coord_accuracy = np.ones((self.outDim,),dtype = np.float32)*512
            for epoch in range(nEpochs):
                ...
                for i in range(epochSize):
                    ...
                    ## data input    
                    y_stack_mini = np.zeros((self.batchSize,self.nStack,self.output_height,self.output_width,self.outDim))
                    vis_stack_mini = np.zeros((self.batchSize,self.nStack,self.output_height,self.output_width,self.outDim))
                    X_train_mini,y_train_mini,coords_mini,vis_mini = self.data_stream_train.get_next_batch()

                    for i_stack in range(self.nStack):
                ...
                weight_summary = self.Session.run(self.weight_op, feed_dict = feed_dict)
                ...
                for i_df_valid in np.arange(0,self.data_stream_valid.df_size,self.batchSize):
                    X_valid_mini,y_valid_mini,coords_mini,vis_mini = self.data_stream_valid.get_next_batch_no_random()
                    vis_stack_mini = np.zeros((self.batchSize,self.nStack,self.output_height,self.output_width,self.outDim))
                    y_stack_mini = np.zeros((self.batchSize,self.nStack,self.output_height,self.output_width,self.outDim))
                    for i in range(self.nStack):
                        y_stack_mini[:,i,:,:,:] = y_valid_mini
                ...
                acc_list = acc_list[1:,:]
                valid_acc_list_mean = np.nanmean(acc_list,axis=0)
                print("VALID accu")
                print(valid_acc_list_mean.astype(int))
                print(np.nanmean(valid_acc_list_mean))
                test_summary = self.Session.run(self.test_op, 
                    feed_dict = {self.joint_accur_v2:valid_acc_list_mean})
                # Save summary (Loss + Accuracy)
                self.test_summary.add_summary(test_summary, global_step)
    ...
    def train_performance(self):
        ...
        for epoch in range(1):
            ...
            for i in range(10):
                ...
                if i==1:
                    print('2 done in ' + str(time.time()-epochstartTime)+ ' sec.')
            epochfinishTime = time.time()
            #Save Weight (axis = epoch)
            weight_summary = self.Session.run(self.weight_op, feed_dict = feed_dict)
            self.train_summary.add_summary(weight_summary, epoch)
